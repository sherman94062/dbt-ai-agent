...
def render_pipeline_tab(context: dict):
    ...
    st.markdown("### 🔧 Full Data Pipeline: From Raw Data to Semantic Layer")
    st.markdown("See how data flows from generation through dbt transformations")
    ...
    ```
    ┌─────────────────┐      ┌─────────────────┐      ┌─────────────────┐      ┌─────────────────┐
    │   load.py       │      │  dbt models/    │      │  dbt build      │      │  PostgreSQL     │
    │   (Python)      │  →   │  (SQL + Jinja)  │  →   │  (compile+run)  │  →   │  (tables/views) │
    │   CREATE TABLE  │      │  {{ ref() }}    │      │  resolved SQL   │      │  query-ready    │
    └─────────────────┘      └─────────────────┘      └─────────────────┘      └─────────────────┘
         Step 1                   Step 2                   Step 3                   Step 4
       Raw Tables              Source Files              Compilation              Execution
    ```
    """)
    ...
    st.markdown("## 🟡 Step 1: Raw Table Creation (load.py)")
    st.markdown("""
    Before dbt runs, we need raw data in PostgreSQL. The `load.py` script:
    1. Uses DuckDB to generate TPC-H benchmark data
    2. Creates tables in PostgreSQL with `CREATE TABLE`
    3. Bulk loads data using `COPY`
    """)
    ...
    with col2:
        st.markdown("**Source File: `load.py`**")
        load_path = Path("load.py")
        if load_path.exists():
            st.code(load_path.read_text(), language="python")
        else:
            st.warning("load.py not found")

    # Show example CREATE TABLE
    with st.expander("🔍 Example: What SQL runs in PostgreSQL", expanded=False):
        st.code("""
-- Generated by load.py for the 'orders' table:
...
-- Then bulk load via COPY command
COPY orders FROM stdin;
        """, language="sql")

    st.markdown("---")

    # Step 2: dbt Source Files
    st.markdown("## 🔵 Step 2: Staging Models (dbt Views)")
    st.markdown("""
